...
<div class="col-md-6">
	<div class="panel panel-default question-box">
	  <div class="panel-heading bg-primary text-white " style="height:80px  !important" >
        <h3 class="panel-title question "><?php echo ("QUESTION : ".$question["question"]);
				echo("<br>MARKS : ".$competency);?></h3>
				<!-- <div class="progress">
				<div class="progress-bar w-75" role="progressbar" aria-valuenow="<?php
				echo $competency
				?>" aria-valuemin="0" aria-valuemax="100"></div>
				</div> -->
	    <h3 class="panel-title info text-warning"><?php echo $_SESSION["question"]?> off 8</h3>
	  		...
	  		<div class="form-group" id="answers-group">
	   			...
	   			<?php while($row = mysqli_fetch_assoc($answers)) { ?>
	  				...
	  				<span class="radio col-md-6">
						<input class="radio_buttons optional text-info" id="answer<?php echo $row['id'] ?>" name="selected_answer" type="radio" value="<?php echo $row['id'] . '|' . $row['is_correct'] ?>"> <?php echo $row['answer'] ?>
		  			</span>
	 
			    <?php } ?> 
	  		</div>
	  		<button class="btn btn-default text-white bg-success "  style="height:35px;width:60px;  !important" type="submit" name="submit" id="submit-answer">
	  			Submit
	  		</button>
	  	</form>
	  </div>
